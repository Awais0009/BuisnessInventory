...
export async function middleware(request: NextRequest) {
  ...
  // If user is logged in and tries to access auth pages or landing, redirect to home
  if (user && (request.nextUrl.pathname.startsWith('/auth/') || request.nextUrl.pathname === '/landing')) {
    // Exception: allow callback page
    if (!request.nextUrl.pathname.startsWith('/auth/callback')) {
      const url = request.nextUrl.clone()
      url.pathname = '/'
      return NextResponse.redirect(url)
